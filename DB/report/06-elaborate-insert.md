### Хранимая процедура со сложной вставкой в несколько таблиц

`create_object_with_schedule` - создаёт объект строительства с указанными данными:
- заказчик (создается новый в таблице customer или указывается ID существующего)
- название, ID участка, даты начала и конца строительства
- данные, уникальные для подтипа объекта (задаются через jsonb, создается запись в соответствующей таблице)
- расписание проводимых работ (задаются через jsonb, добавляются в таблицу work_schedule)

Обработка ошибок осуществляется с помощью конструкции `BEGIN ... EXCEPTION ... END`, при вознкновении исключения происходит откат внесённых изменений в таблицы. 

Процедура определена в [файле](../script/07-special-procedures.sql)
```sql
